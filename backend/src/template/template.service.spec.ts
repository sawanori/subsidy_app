...
describe('TemplateService', () => {
  ...
  describe('resolveTemplate', () => {
    ...
    it('should handle loops with limit', async () => {
      ...
      expect(result).not.toContain('<p>Item 3</p>'); // Limited to 2 items
    });
    ...
    it('should reject templates that are too large', async () => {
      const largeTemplate = 'x'.repeat(2 * 1024 * 1024); // 2MB
      
      await expect(
        service.resolveTemplate(largeTemplate, mockContext)
    ...
    it('should handle timeout configuration', async () => {
      ...
      // This test just verifies the timeout option is accepted
  ...
  describe('validateTemplate', () => {
    ...
    it('should handle potentially problematic templates', async () => {
      const template = '{{#if application.title}}<h1>{{application.title}}</h1>'; // Potentially incomplete
    ...
    it('should warn about performance issues', async () => {
      const largeTemplate = '{{application.title}}'.repeat(30000); // Make it larger to trigger warning
  ...
  describe('extractPlaceholders', () => {
    ...
    it('should ignore Handlebars built-in helpers', () => {
      ...
      // Should only extract application.title, not the if/else helpers
  ...
  describe('security features', () => {
    ...
    it('should use safe string helper', async () => {
      ...
      expect(result).not.toContain('<script>');
      expect(result).toMatch(/&[a-z]+;.*script/); // Some form of HTML escaping
    });
